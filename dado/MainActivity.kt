...
fun DadoBotaoImagem(modifier: Modifier = Modifier) {
    ...
    Column(
        ...
        var numeroDigitado = texto.toIntOrNull()
        OutlinedTextField(
            value = texto,
            onValueChange = { texto = it},
            label = { Text("Digite o número sorteado") }
        )
        if (numeroDigitado != null && numeroDigitado == r) {
            exibirToast(contexto = LocalContext.current as ComponentActivity, mensagem = "Você venceu!!!")
        }
        else {
